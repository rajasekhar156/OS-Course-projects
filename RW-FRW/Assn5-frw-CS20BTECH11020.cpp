...
float MUIcs,MUIrem;
FILE *frw_log = fopen("FRW-log.txt","w");
FILE *avg_log = fopen("Average_FRW_time.txt","w+");
sem_t mut,rwmut ,QUeue;
...
int main()
{
    ...
    Dcs = exponential_distribution<double>(1000/MUIcs);
    Drem = exponential_distribution<double>(1000/MUIrem);
    sem_init(&mut,0,1);
    sem_init(&rwmut,0,1);
    sem_init(&QUeue,0,1);
    ...
    fprintf(avg_log, "Average Time Taken for thread to gain entry in FRW is %f" , (w_avgtime + r_avgtime)/(nw*kw*1000 + nr*kr*1000));
